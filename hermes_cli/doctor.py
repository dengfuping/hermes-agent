...
def run_doctor(args):
    ...
    elif _active_memory_provider == "m0":
        try:
            ...
            if not m0_key:
                _fail_and_issue(
                    "M0 API key not set",
                    "(set M0_API_KEY in .env or run: hermes memory setup)",
                    "M0 is set as memory provider but API key is missing",
                    issues,
                )
            else:
                ...
                try:
                    ...
                    _fail_and_issue(
                        "M0 status check failed",
                        f"HTTP {he.code} — verify AK and base URL",
                        f"M0 unreachable: HTTP {he.code}",
                        issues,
                    )
                except Exception as _e:
                    _fail_and_issue("M0 status check failed", str(_e), f"M0 unreachable: {_e}", issues)
        except Exception as _e:
